...
var CalHeatMap = function () {
	...
	function _initCalendar() {
		...
		if (self.options.loadOnInit) {
			self.getDatas(
				...
				function () {
					//self.options.computeTotalCntByLegend();
					self.Legend.redraw(self.graphDim.width - self.options.domainGutter - self.options.cellPadding);
	...
	this.paint = function (navigationDir) {
		...
		// =========================================================================//
		// PAINTING CUSTOM LABEL															//
		// =========================================================================//
		function addCustomLable(labelName, labelPos, textAnchor) {
			domainSvg.append("g")
				.selectAll("text")
				.data(labelName)
				.enter()
		...
		var weekdaysLableName = ["Tue", "Thu", "Sat"];
		function getWeekdaysLablePos() {
			var weekdayXPos = options.domainMargin[3]-8;
			var cellSzPa = options.cellSize + options.cellPadding;
			var tueYPos = options.domainMargin[0] + cellSzPa + cellSzPa/2;
			var thuYPos = tueYPos + cellSzPa*2;
			var satYPos = thuYPos + cellSzPa*2;
			return [[weekdayXPos, tueYPos], [weekdayXPos, thuYPos], [weekdayXPos, satYPos]];
		}
		var weekdaysLablePos =  getWeekdaysLablePos();
		addCustomLable(weekdaysLableName, weekdaysLablePos, "end");

		var monthsLableName = ["Jan", "Feb", "Mar", "Apr", "May", "June", "July", "Aug", "Sep", "Oct", "Nov", "Dec"];
		
		function getMonList(y){
			var monlist = [];
			for (var i = 0; i < 12; i++){
				monlist.push(d3.time.monday.range(new Date(y, i, 1), new Date(y, i, 8))[0]);
			}
			return monlist;
		}

		function getMonthsLablePos(){
			var monthsYPos = options.domainMargin[0]-10;
			var mondays = getMonList(2021);
			//console.log(mondays);
			var cellSzPa = options.cellSize + options.cellPadding;
			return mondays.map(function(x){
				var xpos = self.positionSubDomainX(x) + options.domainMargin[3];
				return [xpos, monthsYPos];
			});
		}
		var monthsLablePos = getMonthsLablePos();
		addCustomLable(monthsLableName, monthsLablePos, "start");
...
Legend.prototype.computeDim = function () {
	...
	var textWidth = options.legendDesc.map(function (s) {
		return textSize(s).width;
	});

	options.legendTxtWidth = textWidth;

	var total = 0;
	var idx = 0;
	for (idx = 0; idx < options.legendDesc.length; idx++) {
		options.legendXPos[idx] = total;
		total = total + options.legendCellSize + options.legendCellPadding*10 + textWidth[idx];
	}
	options.legendXPos[idx] = total;
...
Legend.prototype.redraw = function (width) {
	...
	legendItem
		...
		.append("text")
		.text(function (d, i) {
			return options.legendDesc[i];
		})
		.call(legendTextLayout)
		.attr("class", "graph-label")
		.attr("dominant-baseline", "hanging")
		.attr("fill-opacity", 1)
		.call(function (selection) {
	...
	function legendTextLayout(selection) {
		selection
			.attr("width", function (d, i) {
				return options.legendTxtWidth[i];
			})
			.attr("height", options.legendCellSize)
			.attr("x", function (d, i) {
				return options.legendXPos[i] + options.legendCellSize + options.legendCellPadding;
			})
			;
	}
	...
	function getLegendXPosition() {
		switch (options.legendHorizontalPosition) {
			...
			case "middle":
			case "center":
				return Math.round(width / 2 - parent.getDim("width") / 2) + options.domainMargin[3];
			default:
...
Legend.prototype.buildColors = function () {
	...
	console.log(this.calendar.legendScale);
	return true;
};

function getLegendIndex(n, legend) {
	"use strict";
	if (n === null || isNaN(n)) {
		return -1;
	}
	var idx = legend.length;
	for (var i = 0, total = legend.length - 1; i <= total; i++) {
		if (legend[0] > 0 && n < 0) {
			idx = 0;
			break;
		}
...
Legend.prototype.totalCntByLegendIdx = function(){
	"use strict";

	var _domains = this.calendar._domains;
	var legend = this.calendar.options.legend;
	var totalCntByLegendIdx = this.calendar.options.totalCntByLegendIdx;
	//console.log(this);
	_domains.forEach(function(k,v){
		v.forEach(function(elem){
			//console.log(JSON.stringify(elem));
			//console.log(elem);
			if(elem.v != null){
				//console.log(JSON.stringify(elem));
				var idx = getLegendIndex(elem.v, legend);
				if (idx >=0 )totalCntByLegendIdx[idx]++;
			}
		})
	})

	this.calendar.options.legendDesc.forEach(function(e,i,a){
		a[i] = a[i] + " (" + totalCntByLegendIdx[i] + ")"
	})
	console.log(this.calendar.options.legendDesc);
}
...
function textSize(text) {
	...
	container.append('text')
	  .style("font-size", "10px")      // todo: these need to be passed to the function or a css style
	  .style("font-family", "sans-serif")
	  .text(text);
  
	let sel = container.selectAll('text').node()
	let width = Math.ceil(sel.getComputedTextLength())
	let height = Math.ceil(sel.getExtentOfChar(0).height)
	container.remove()
	return {width, height}
  }
